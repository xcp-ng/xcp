...
commit_match_info_repr = {
    CommitMatchInfoFlag.SameCommit: CommitMatchInfo(
        CommitMatchInfoFlag.SameCommit,
...
def is_ancestor(
        repo: pygit2.Repository,
        potential_ancestor: pygit2.Commit,
        of_commit: pygit2.Commit
) -> bool:
...
class BranchRange(object):
    ...
    def __init__(
        ...
        if merge_base is not None and not is_ancestor(
                repo,
                repo.get(self.start_range_oid),
                repo.get(merge_base)
        ):
    ...
    def init_range(self) -> None:
        ...
        for commit in range_log(
                self._repo,
                self.merge_base or self.start_range_oid,
...
class RebasedCommitsMatches(object):
    ...
    def init_matches(self) -> None:
        ...
        for left_commit_oid in self.left_range._commit_by_oid:
            ...
            right_commit = self.right_range._commit_by_oid.get(
                left_commit_oid
            )
        ...
        for commit_oid, rebased_commit_match in (
            ...
            self.add_row(
                ...
                markers_from_match_type(
                    rebased_commit_match.match_info
                ),
                right_cell,
                key=commit_oid
            )
