...
#"${DNF[@]}" --downloadonly --downloaddir="$SCRATCHDIR" \
#	    download memtest86+
(cd "$SCRATCHDIR" && wget http://www.rpmfind.net/linux/openmandriva/cooker/repository/x86_64/main/release/memtest86+-6.00-1-omv4090.x86_64.rpm)
rpm2cpio $SCRATCHDIR/memtest86+-*.rpm | (cd $ISODIR && cpio ${VERBOSE} -idm "./boot/*")
mv ${VERBOSE} $ISODIR/boot/memtest.bin $ISODIR/boot/isolinux/memtest
